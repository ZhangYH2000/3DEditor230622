...
proc parsing_def(vector<bra> const& in) {
	...
	p.def_fun = in[1].tk.var;
	bool comma = true;
	for (auto const& b : in[2].bras) {
		if (comma) {
			comma = false; p.def_pars.push_back(b.tk.var);
			if (b.tk.typ != L"var") { return proc(L"error"); }
		}
		else {
			comma = true;
			if (b.tk.typ != L",") { return proc(L"error"); }
		}
	}
...
proc parsing_ret(vector<bra> const& in) {
	...
	p.ret = tmp.empty() ? expr(L"null") : parsing_expr(tmp);
	return p;
}
proc parsing_assign(vector<bra> const& in) {
	...
	auto it = in.begin();
	for (; it != in.end(); ++it) {
		if (it->tk.typ == L"=") { break; }
	}
	auto tmp = vector<bra>(in.begin(), it);
	p.left = parsing_expr(tmp);
	tmp = vector<bra>(it + 1, in.end());
	p.right = parsing_expr(tmp);
	return p;
}
proc parsing_if(vector<bra> const& in) {
	...
	if (in.size() != 3) {
		...
		if (in[3].tk.typ != L"else" || in[4].typ != 2) {
			return proc(L"error");
		}
...
vector<proc> parsing_procs(vector<bra> const& in) {
	...
	for (auto const& b : in) {
		...
		if (b.tk.typ == L";") {
			if (!cur.empty()) { out.push_back(parsing_proc(cur)); }
			cur.clear();
		}
		else {
