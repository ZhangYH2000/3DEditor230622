...
(vector<pre_expr> const& in, expr& out, vector<wstring> const& mark) {
	rep(i, 0, in.size()) {
		...
		if (!pre.istk) { continue; }
		auto const& s = pre.tk.typ;
		auto it = find(mark.begin(), mark.end(), s);
		if (it != mark.end()) {
			bool bad =
				i == 0 || i == in.size() - 1 ||
...
expr parsing_bins(vector<bra> const& in) {
	...
	if (!tmp.empty()) {
		pres.push_back(pre_expr(parsing_expr(tmp)));
	}
...
expr parsing_access(vector<bra> const& in) {
	expr ex(L"access");
	auto loc = vector<bra>(in.begin(), in.end() - 1);
	ex.loc = parsing_expr(loc);
	ex.ind = parsing_expr(in.back().bras);
	return ex;
}
