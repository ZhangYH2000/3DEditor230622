...
void draw_str(tile& dest, dbuf& ds, double d, wstring const& s,
	...
	for (auto c : s) {
		if (c == '\n') { tl_cur.y += ft.h; x_cur = 0; }
			...
			bool nxt_line = w != 0 &&
				x_cur > 0 && x_cur + t.w > w;
			if (nxt_line) { tl_cur.y += ft.h; x_cur = 0; }

			dvec tmp = tl_cur + dvec(x_cur, 0);
			draw_char(dest, ds, d, tmp, vp, t, col);
			x_cur += t.w;
		}
	}
}
...
int str_index_at_pos
(wstring const& s, font const& ft, dvec p, int w) {
	int x_cur = 0, y_cur = 0;
	if (0 > p.y) { return 0; }
	...
	rep(ind, 0, s.size()) {
		...
		if (c == L'\n') {
			...
			bool over = x_cur > p.x
				&& y_cur + ft.h > p.y;
			if (over) {
				bool left = x_cur - p.x > t.w / 2;
				return left ? ind : ind + 1;
			}
		}
	}
	return s.size();
