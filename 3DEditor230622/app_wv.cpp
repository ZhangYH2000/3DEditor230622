...
bool wvblk::play(HWAVEOUT hwo) {
	MMRESULT ret0, ret1;
	ret0 = waveOutPrepareHeader(hwo, &hdr, sizeof(hdr));
	ret1 = waveOutWrite(hwo, &hdr, sizeof(hdr));
	return
		ret0 == MMSYSERR_NOERROR &&
		ret1 == MMSYSERR_NOERROR;
}
bool wvblk::finish(HWAVEOUT hwo) {
	auto ret = waveOutUnprepareHeader(hwo, &hdr, sizeof(hdr));
...
bool app_wv::init() {
	...
	// 好像这里不能是其它数了，没怎么搞清楚。
	wofm.wBitsPerSample = 8;
	wofm.nBlockAlign = 1;
	wofm.nAvgBytesPerSec = sp;

	MMRESULT ret = waveOutOpen
	(&hwo, WAVE_MAPPER, &wofm, NULL, 0, CALLBACK_NULL);
...
bool app_wv::play() {
	...
	for (auto v : wvin) {
		wvblks[n_play]->dat[cur++] =
			clmp(128 + int(v * 128), 0, 255);
		if (cur == len_block_n()) {
			bool ret = wvblks[n_play++]->play(hwo);
			if (!ret) { return false; } cur = 0;
		}
	}
	wvin = vector<double>(n_in * len_block_n()); return true;
}
void app_wv::exit() {
	while (n_play > 0) {
