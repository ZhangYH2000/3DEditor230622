...
FILE* wfopen(wstring const& nm, wstring const& arg) {
	FILE* f = NULL;
	_wfopen_s(&f, nm.c_str(), arg.c_str());
	return f;
}
wstring read_txt(wstring const& nm) {
	FILE* f = wfopen(nm, L"r");
	if (!f) { return {}; }

	// 注意 WEOF 不要写成 EOF。
	wstringstream s; wchar_t c = getwc(f);
	while (c != WEOF) { s << c; c = getwc(f); }
	fclose(f); return sift_chs(s.str());
}
